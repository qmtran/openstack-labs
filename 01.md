...
4. Collect the versions of various OpenStack services
...
5. Now navigate in browser to: [OpenStack Releases](https://wiki.openstack.org/wiki/Releases)

6. Look over the chart and notice that the release date of Kilo is the same date that was being displayed each time you issued the `[$SERVICE]-manage --version command`. OpenStack is a project with ambition release dates. Take a minute to read over the notes concerning how the project is maintained.
